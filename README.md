...
A Contributor role creates and edits content but cannot publish a Layout, delete anything, or put content on a screen, so a student club or a volunteer can build a slide that somebody checks first. Content review has no submit step: a playlist or Layout is waiting for review whenever its current revision has no decision, which means editing approved content sends it back automatically instead of relying on every edit path to remember. Screen scopes narrow an account to named buildings or sync groups for screen operations, while leaving the shared content library alone; no grants means the whole fleet, and an Owner is never scoped. See [Content review](docs/content-review.md) and [Screen scopes](docs/screen-scopes.md).

Snapshot history keeps bounded periodic images of what screens showed, so "what was on it at 10:14" is answerable over a break or after a complaint. It is off by default, capped three independent ways, stored in the database and therefore inside backups, and every surface states what it proves: the Player rendered that frame, not that the television was on or on the right input. Playlists now keep a revision history with restore, as Layouts already did; restoring makes a new revision rather than rewinding, and deleted media is skipped and counted rather than resurrected. See [Snapshot history](docs/snapshots.md) and [Playlist history](docs/playlist-history.md).
