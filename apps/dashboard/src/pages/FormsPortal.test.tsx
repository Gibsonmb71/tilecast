...
const workflow: FormWorkflow = {
  states: [
    ...
    { key: "rejected", label: "Rejected", position: 4, eligibleForOutput: false, initial: false, terminal: true },
  ],
  transitions: [
    { from: "draft", to: "submitted", label: "Submit", requiredCapability: "submit", position: 0 },
    { from: "submitted", to: "approved", label: "Approve", requiredCapability: "approve", position: 1 },
    { from: "submitted", to: "changes_requested", label: "Request changes", requiredCapability: "review", position: 2 },
    { from: "changes_requested", to: "submitted", label: "Resubmit", requiredCapability: "submit", position: 3 },
  ],
};
...
function form(capabilities: FormDataSource["grantedCapabilities"]): FormDataSource {
  return {
    id: "f1",
...
function detail(overrides: Partial<FormRecordDetail> = {}): FormRecordDetail {
  return {
    ...
    availableTransitions: [
      { to: "submitted", toLabel: "Submitted", label: "Submit", requiredCapability: "submit", requiresNote: false },
    ],
    ...overrides,
  };
}
...
function renderPortal(path: string) {
  ...
  const router = createMemoryRouter(
    [
      ...
      { path: "/forms/:id/submissions/:recordId", element: <FormPortalSubmissionPage /> },
    ],
    { initialEntries: [path] },
  );
  render(
...
describe("Forms portal", () => {
  it("lists accessible forms with submission counts", async () => {
    ...
    const summary: FormSummary = {
      ...
      submissionCounts: { draft: 2, submitted: 1, changesRequested: 3, total: 6 },
    };
    vi.spyOn(api, "listForms").mockResolvedValue([summary]);
    renderPortal("/forms");

    expect(await screen.findByRole("heading", { name: "My Forms" })).toBeInTheDocument();
  ...
  it("creates a draft and submits via the server-provided transition", async () => {
    ...
    vi.spyOn(api, "listFormRecords").mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 100 });
    const create = vi
      .spyOn(api, "createFormRecord")
      .mockResolvedValue(record({ id: "rec9", version: 1, state: "draft" }));
    const transition = vi
      .spyOn(api, "transitionFormRecord")
      .mockResolvedValue(record({ id: "rec9", version: 2, state: "submitted" }));
    const user = userEvent.setup();
    renderPortal("/forms/f1/new");
    ...
    expect(create.mock.calls[0]![1].values).toMatchObject({ title: "Big news" });
    await waitFor(() => expect(transition).toHaveBeenCalled());
    expect(transition.mock.calls[0]![2]).toMatchObject({ toState: "submitted", version: 1 });
  });
  ...
  it("shows the reviewer note and resubmits a changes-requested submission", async () => {
    ...
    vi.spyOn(api, "getFormRecord").mockResolvedValue(
      detail({
        ...
        comments: [
          { id: "c1", authorName: "Reviewer", body: "Please add a date", createdAt: "2026-01-03T00:00:00Z" },
        ],
        availableTransitions: [
          { to: "submitted", toLabel: "Submitted", label: "Resubmit", requiredCapability: "submit", requiresNote: false },
        ],
      }),
    );
    const update = vi
    ...
    expect((await screen.findAllByText("Please add a date")).length).toBeGreaterThan(0);
    await user.click(screen.getByRole("button", { name: "Resubmit" }));

    await waitFor(() => expect(update).toHaveBeenCalled());
    await waitFor(() => expect(transition).toHaveBeenCalled());
    expect(transition.mock.calls[0]![2]).toMatchObject({ toState: "submitted" });
  });
  ...
  it("uploads a selected image after creating the draft", async () => {
    ...
    const withImage = {
      ...schema,
      fields: [...schema.fields, { key: "photo", label: "Photo", control: "image" as const }],
    };
    imageForm.publishedRevision = { ...imageForm.publishedRevision!, schema: withImage };
    vi.spyOn(api, "getForm").mockResolvedValue(imageForm);
    vi.spyOn(api, "listFormRecords").mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 100 });
    vi.spyOn(api, "createFormRecord").mockResolvedValue(record({ id: "rec9", version: 1 }));
    const upload = vi
      .spyOn(api, "uploadFormRecordAttachment")
      .mockResolvedValue(detail({ id: "rec9", version: 2, values: { title: "Hi", photo: "asset-1" } }));
    const user = userEvent.setup();
    renderPortal("/forms/f1/new");
