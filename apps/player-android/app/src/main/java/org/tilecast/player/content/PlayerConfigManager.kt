...
class PlayerConfigManager(private val database:PlayerDatabase,private val api:TilecastApi){
    ...
    suspend fun clear() { database.playerConfigs().clear(); activeConfigVerified = false }
    suspend fun loadActive():PlayerConfig? {
        val stored = database.playerConfigs().active() ?: run { activeConfigVerified = false; return null }
        val config = runCatching { api.decodePlayerConfig(stored.rawJson).also(PlayerConfigValidator::validate) }.getOrNull()
            ?: run { activeConfigVerified = false; return null }
        activeConfigVerified = true
        return config
    }
    suspend fun reconcile(server:String,credential:String):PlayerConfig? {
        val active=database.playerConfigs().active()
        val response=api.playerConfig(server,credential,playerConfigEtagForRequest(activeConfigVerified,active?.etag))
        if(response.notModified)return null
        val config=response.config?:return null
        PlayerConfigValidator.validate(config)
        if(!shouldAcceptPlayerConfig(active?.configRevision,config.configRevision))return null
        val raw=response.rawJson?:return null
        database.withTransaction{database.playerConfigs().save(StoredPlayerConfig(config.configRevision,config.schemaVersion,raw,response.etag,"ready",System.currentTimeMillis()));database.playerConfigs().activate(config.configRevision,System.currentTimeMillis())}
        activeConfigVerified = true
        return config
    }
}
