...
data class UpdateUiState(val deploymentId:String,val currentVersion:String,val newVersion:String,val state:String,val downloadedBytes:Long,val expectedBytes:Long,val message:String,val permissionRequired:Boolean=false,val installReady:Boolean=false,val maintenanceAt:String?=null)
...
object PlayerUpdateVerifier {
    fun validate(remote:PlayerUpdateMetadata,currentVersionCode:Long,sdk:Int,archive:ArchiveMetadata){
...
class PlayerUpdateManager(private val app:Application,private val api:TilecastApi){
    ...
    suspend fun prepare(server:String,credential:String,command:PlayerCommand,emergencyActive:()->Boolean,onState:(UpdateUiState)->Unit):CommandOutcome{
        ...
        return try{
            ...
            PlayerUpdateVerifier.validate(metadata,BuildConfig.VERSION_CODE.toLong(),Build.VERSION.SDK_INT,archive)
    ...
    fun install(state:UpdateUiState):Boolean{
        ...
        return runCatching{
            val params=PackageInstaller.SessionParams(PackageInstaller.SessionParams.MODE_FULL_INSTALL).apply{setAppPackageName(BuildConfig.APPLICATION_ID)}
    ...
    private fun persist(state:UpdateUiState){store.edit().putString("deployment",state.deploymentId).putString("new-version",state.newVersion).putString("state",state.state).putLong("downloaded",state.downloadedBytes).putLong("expected",state.expectedBytes).putBoolean("permission",state.permissionRequired).putBoolean("ready",state.installReady).apply{if(state.maintenanceAt==null)remove("maintenance-at") else putString("maintenance-at",state.maintenanceAt)}.apply()}
}
