...
class ReliabilityController(private val context:Context) {
    ...
    fun setAdminPin(pin:CharArray){val stored=AdminPinGate().create(pin);store.edit().putString("admin-pin-salt",Base64.encodeToString(stored.salt,Base64.NO_WRAP)).putString("admin-pin-hash",Base64.encodeToString(stored.hash,Base64.NO_WRAP)).putInt("admin-pin-iterations",stored.iterations).putLong("admin-pin-changed-at",System.currentTimeMillis()).commit();pin.fill('\u0000')}
