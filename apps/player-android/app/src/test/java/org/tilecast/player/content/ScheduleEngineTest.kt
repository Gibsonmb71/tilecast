...
class ScheduleEngineTest {
	@Test fun pendingManifestUsesServerActivationGrace(){assertEquals(30_000L,pendingActivationDelayMillis(30));assertEquals(30_000L,pendingActivationDelayMillis(0));assertEquals(3_600_000L,pendingActivationDelayMillis(9_999))}
	@Test fun scheduledPresentationDoesNotMixWithFallback(){val at=Instant.parse("2026-07-12T16:00:00Z");val layout=ManifestSchedule("layout",null,"one_time","UTC",1,1,layoutId="scheduled-layout",oneTimeStart="2026-07-12T15:00:00Z",oneTimeEnd="2026-07-12T17:00:00Z");val selectedLayout=ScheduleEngine.resolve(at,listOf(layout),"fallback-playlist","fallback-layout");assertNull(selectedLayout.playlistId);assertEquals("scheduled-layout",selectedLayout.layoutId);val playlist=layout.copy(id="playlist",playlistId="scheduled-playlist",layoutId=null);val selectedPlaylist=ScheduleEngine.resolve(at,listOf(playlist),"fallback-playlist","fallback-layout");assertEquals("scheduled-playlist",selectedPlaylist.playlistId);assertNull(selectedPlaylist.layoutId)}
