package auth

import (
	"encoding/hex"
	"fmt"
	"strings"

	"github.com/go-webauthn/webauthn/protocol"
	"github.com/go-webauthn/webauthn/webauthn"
)

// Every authenticator reports an AAGUID identifying its make and model, which
// is how a passkey can be given a name the user recognizes without asking them
// to invent one. The AAGUID appears only in the registration response, never in
// an assertion, so it has to be resolved at enrollment.
//
// Names come from the community list at
// https://github.com/passkeydeveloper/passkey-authenticator-aaguids. Only the
// names are embedded: the upstream file is mostly base64 icons, and a
// self-hosted server cannot fetch it at runtime. An unknown AAGUID is expected
// and falls back to the authenticator's attachment.
var passkeyProviders = map[string]string{
	"bada5566-a7aa-401f-bd96-45619a55120d": "1Password",
	"a11a5faa-9f32-4b8c-8c5d-2f7d13e8c942": "AliasVault",
	"fbfc3007-154e-4ecc-8c0b-6e020557d7bd": "Apple Passwords",
	"a4a2d88e-9796-4356-9164-e2a5a8bd019c": "Avast Password Manager",
	"6bb49926-160a-4306-a100-4eb39ba6ac45": "AVG Password Manager",
	"e7db2bd3-f2fe-4d71-ad78-7e7aa166cfd1": "Avira Password Manager",
	"d548826e-79b4-db40-a3d8-11116f7e8349": "Bitwarden",
	"c9cadfc9-89a9-489e-a25a-c7e86a4d5f15": "Burp Suite Navigation Recorder",
	"adce0002-35bc-c60a-648b-0b25f1f05503": "Chrome on Mac",
	"b5397666-4885-aa6b-cebf-e52262a439a2": "Chromium Browser",
	"531126d6-e717-415c-9320-3d9aa6981239": "Dashlane",
	"de503f9c-21a4-4f76-b4b7-558eb55c6f89": "Devolutions",
	"771b48fd-d3d4-4f74-9232-fc157ab0507a": "Edge on Mac",
	"f3809540-7f14-49c1-a8b3-8f813b225541": "Enpass",
	"d2717a32-9851-48a8-9961-b264c97a411a": "Fenko Vault",
	"ea9b8d66-4d01-1d21-3ce4-b6b48cb575d4": "Google Password Manager",
	"d49b2120-b865-4191-8cea-be84a52b0485": "Heimlane Vault",
	"da583154-ce16-4cdf-9fe6-1dba788c0998": "Hey Be Safe",
	"dd4ec289-e01d-41c9-bb89-70fa845d4bf2": "iCloud Keychain (Managed)",
	"39a5647e-1853-446c-a1f6-a79bae9f5bc7": "IDmelon",
	"6d212b28-a2c1-4638-b375-5932070f62e9": "initial",
	"bfc748bb-3429-4faa-b9f9-7cfa9f3b76d0": "iPasswords",
	"a10c6dd9-465e-4226-8198-c7c44b91c555": "Kaspersky Password Manager",
	"eaecdef2-1c31-5634-8639-f1cbd9c00a08": "KeePassDX",
	"9addb28c-b46f-4402-808f-019651441ff3": "KeePassPasskey",
	"fdb141b2-5d84-443e-8a35-4698c205a502": "KeePassXC",
	"0ea242b4-43c4-4a1b-8b17-dd6d0b6baec6": "Keeper",
	"b78a0a55-6ef8-d246-a042-ba0f6d55050c": "LastPass",
	"22248c4c-7a12-46e2-9a41-44291b373a4d": "LogMeOnce",
	"d3452668-01fd-4c12-926c-83a4204853aa": "Microsoft Password Manager",
	"b84e4048-15dc-4dd0-8640-f4f60813c8af": "NordPass",
	"fa37f553-f9b6-4adb-ac53-8bbb57ebdf0d": "Norton Password Manager",
	"70617373-7761-6c6c-6669-646f32303236": "Passwall",
	"50726f74-6f6e-5061-7373-50726f746f6e": "Proton Pass",
	"d350af52-0351-4ba2-acd3-dfeeadc3f764": "pwSafe",
	"53414d53-554e-4700-0000-000000000000": "Samsung Pass",
	"e8b7f4a2-c3d5-e6f7-890a-b1c2d3e4f567": "Sherlocked",
	"d9be9d39-e6a6-4c28-a581-32b044d986e4": "Sticky Password Manager",
	"891494da-2c90-4d31-a9cd-4eab0aed1309": "S\u00e9same",
	"8836336a-f590-0921-301d-46427531eee6": "Thales Bio Android SDK",
	"66a0ccb3-bd6a-191f-ee06-e375c50b9846": "Thales Bio iOS SDK",
	"cd69adb5-3c7a-deb9-3177-6800ea6cb72a": "Thales PIN Android SDK",
	"17290f1e-c212-34d0-1423-365d729f09d9": "Thales PIN iOS SDK",
	"cc45f64e-52a2-451b-831a-4edd8022a202": "ToothPic Passkey Provider",
	"08987058-cadc-4b81-b6e1-30de50dcbe96": "Windows Hello",
	"9ddd1817-af5a-4672-a2b9-3e3dd95000a9": "Windows Hello",
	"6028b017-b1d4-4c02-b4b3-afcdafc96bb2": "Windows Hello",
	"b35a26b2-8f6e-4697-ab1d-d44db4da28c6": "Zoho Vault",
}

// A zero AAGUID means the authenticator declined to identify itself, which is
// what iCloud Keychain and several browsers do. It is not an error.
const zeroAAGUID = "00000000-0000-0000-0000-000000000000"

func formatAAGUID(raw []byte) string {
	if len(raw) != 16 {
		return ""
	}
	value := hex.EncodeToString(raw)
	return fmt.Sprintf("%s-%s-%s-%s-%s", value[0:8], value[8:12], value[12:16], value[16:20], value[20:32])
}

// describePasskey names a credential from what the authenticator reported.
// The AAGUID is preferred; failing that, attachment and transports still
// distinguish "the device in your hand" from "a key on your keyring", which is
// the distinction a user actually needs when deciding which one to remove.
func describePasskey(credential *webauthn.Credential) string {
	aaguid := formatAAGUID(credential.Authenticator.AAGUID)
	if aaguid != "" && aaguid != zeroAAGUID {
		if name, ok := passkeyProviders[aaguid]; ok {
			return name
		}
	}
	switch credential.Authenticator.Attachment {
	case protocol.Platform:
		return "This device"
	case protocol.CrossPlatform:
		for _, transport := range credential.Transport {
			switch transport {
			case protocol.USB, protocol.NFC, protocol.BLE:
				return "Security key"
			}
		}
		return "Another device"
	}
	return "Passkey"
}

// uniquePasskeyName keeps two passkeys from the same provider distinguishable.
// Numbering only starts at the second one, so the common case stays clean.
func uniquePasskeyName(name string, existing []PasskeySummary) string {
	taken := make(map[string]bool, len(existing))
	for _, passkey := range existing {
		taken[strings.ToLower(passkey.Name)] = true
	}
	if !taken[strings.ToLower(name)] {
		return name
	}
	for suffix := 2; suffix < 100; suffix++ {
		candidate := fmt.Sprintf("%s (%d)", name, suffix)
		if !taken[strings.ToLower(candidate)] {
			return candidate
		}
	}
	return name
}
