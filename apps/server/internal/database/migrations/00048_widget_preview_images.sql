...
ALTER TABLE widgets
    ...
    ADD CONSTRAINT widgets_preview_image_size CHECK (preview_image IS NULL OR octet_length(preview_image) BETWEEN 1 AND 512000),
    ADD CONSTRAINT widgets_preview_image_shape CHECK (
        ...
        OR
        (preview_image IS NOT NULL AND preview_content_type = 'image/jpeg' AND preview_width = 960 AND preview_height = 540 AND preview_updated_at IS NOT NULL)
    );

-- +goose Down
ALTER TABLE widgets
    DROP CONSTRAINT widgets_preview_image_shape,
    DROP CONSTRAINT widgets_preview_image_size,
