...
func TestPlaylistAssignmentManifestLifecycle(t *testing.T) {
	...
	if manifest.SchemaVersion != 9 || manifest.DirectFallbackPlaylist == nil || len(manifest.DirectFallbackPlaylist.Items) != 2 || len(manifest.Assets) != 2 {
	...
	if err != nil || webManifest.SchemaVersion != 9 || len(webManifest.Sources) != 1 || webManifest.Sources[0].Provider != "website" || len(webManifest.Assets) != 1 {
