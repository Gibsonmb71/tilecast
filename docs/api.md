...
Built-in plugins are listed at `GET /plugins`. The catalog includes Forms, whose existing form and record endpoints remain below `/forms` and `/data-sources/{id}`, and Player-facing plugins such as Countdown Bar and Brand Bug / Watermark. Countdown Bar instances are managed below `/plugins/countdown-bar/instances` and Brand Bug instances below `/plugins/brand-bug/instances`; Owner or Administrator and CSRF are required for create, replace, and delete. See [Built-in plugins](plugins.md) for plugin behavior and boundaries.
