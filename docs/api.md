...
Definition and workflow (per-form `manage`): `POST /api/v1/forms` creates a form (global `editor`+; the creator becomes its manager). `GET /api/v1/data-sources/{id}/form` returns the form detail with the caller's effective capabilities. `PATCH …/form/draft` edits the unpublished draft schema; `POST …/form/publish` snapshots it into a new immutable revision; `PUT …/form/workflow` configures a bounded, script-free set of states and transitions (default: `draft`, `submitted`, `changes_requested`, `approved`, `rejected`, `expired`). Each state carries an eligible-for-signage flag; only approved is eligible by default.

Records: `GET/POST /api/v1/data-sources/{id}/records` lists (scoped to the caller's own submissions without `view_all`) and creates drafts. `GET/PATCH/DELETE …/records/{recordId}` reads, edits (optimistic concurrency via `version`; conflicts return `409 conflict`), and removes a record. `POST …/records/{recordId}/transitions` runs a workflow transition — authorized on the server for the transition's required capability, validated against current state and record version, and requiring complete required fields when entering an output-eligible state — then records history, invalidates affected manifests, and rebuilds the affected views. `POST …/records/{recordId}/comments` adds a reviewer comment; `POST …/records/{recordId}/attachments` links an uploaded image, reclassifying it so it is never selectable as public Media.
