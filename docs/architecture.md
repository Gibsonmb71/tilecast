...
- `internal/plugins` owns the closed built-in plugin registry, Countdown Bar instances and targets, and per-screen plugin projection. Plugins do not load third-party code and reach the Linux renderer on a channel independent of presentation playback.
