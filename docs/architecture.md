...
Manifest v13 extends that boundary into a declarative presentation runtime. `internal/media` retains the closed authoring/provider registry, while `internal/playlists` compiles Widget configuration into a provider-neutral native node tree or web sandbox descriptor. Android validates capabilities and interprets those documents instead of selecting a renderer from the provider name. A persisted presentation-catalog fingerprint invalidates manifests when Tilecast-shipped compilers change.
