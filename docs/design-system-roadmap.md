...
| Problem                    | Account, create, filter, and timezone menus independently manage placement, outside clicks, Escape, and keyboard behavior. Native `details` elements and custom panels do not provide one consistent menu contract.                  |
| Current examples           | Account menu, operations create menu, asset filter menu, activity advanced filters, and schedule timezone menu.                                                                                                                      |
...
| Migration targets          | Account menu and operations create menu first. asset and activity filters second. timezone selector after select-versus-menu semantics are resolved.                                                                                 |
| Dependencies               | Layering and portal policy, focus restoration, collision-aware placement approach, drawer fallback.                                                                                                                                  |
| Completion criteria        | Action and form-popover modes are separately specified. keyboard matrices cover each mode. positioning works near every viewport edge. two unrelated menus migrate without page-specific dismissal code.                             |
...
| Current examples           | Implemented list toolbars, `ToggleGroup`, and `ViewToggle`. page-specific asset filter portal, activity advanced filters, and active filter chips.                                                                |
...
| Migration targets          | Content and data-source list toolbars first. activity advanced filters second. widgets after common behavior is proven.                                                                                           |
| Dependencies               | Popover foundation, Signal Select, URL-state convention, collection status language.                                                                                                                              |
| Completion criteria        | Filter definitions render consistently in inline and overflow placements. chip removal and clear-all are keyboard accessible. URL restoration is tested. two list pages migrate without losing filter capability. |
